...
import { CalendarEvent, CalendarEventType, Agent } from '../../types'
...
export function AgentCalendar({ agent }: AgentCalendarProps) {
  ...
  // Filter events for this specific agent (deals owned by agent, clients of agent, brands owned by agent)
  const filteredEvents = useMemo(() => {
    if (!allEvents) return []

    // Get client IDs for this agent
    const agentClientIds = new Set(agent.clients?.map(c => c.id) || [])
    // Get brand IDs owned by this agent
    const agentBrandIds = new Set(agent.ownedBrands?.map(b => b.id) || [])
    // Get deal IDs owned by this agent
    const agentDealIds = new Set(agent.deals?.map(d => d.id) || [])

    return allEvents.filter(event => {
      // Include if event is for a client of this agent
      if (event.talentClient && agentClientIds.has(event.talentClient.id)) return true
      // Include if event is for a brand owned by this agent
      if (event.brand && agentBrandIds.has(event.brand.id)) return true
      // Include if event is for a deal owned by this agent
      if (event.deal && agentDealIds.has(event.deal.id)) return true
      return false
    })
  }, [allEvents, agent])
