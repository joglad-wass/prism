...
import { CalendarEvent, CalendarEventType, Brand } from '../../types'
...
export function BrandCalendar({ brand }: BrandCalendarProps) {
  ...
  // Filter events for this specific brand
  const filteredEvents = useMemo(() => {
    if (!allEvents) return []

    // Get deal IDs for this brand
    const brandDealIds = new Set(brand.deals?.map(d => d.id) || [])

    return allEvents.filter(event => {
      // Include if event is for this brand
      if (event.brand && event.brand.id === brand.id) return true
      // Include if event is for a deal of this brand
      if (event.deal && brandDealIds.has(event.deal.id)) return true
      return false
    })
  }, [allEvents, brand])
